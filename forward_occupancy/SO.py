...
# Version with minimized math operations, to evaluate someday
def _make_spheres_minified_script(center_1: torch.Tensor,
        center_2: torch.Tensor,
        radius_1: torch.Tensor,
        radius_2: torch.Tensor,
        center_jac_1: torch.Tensor = torch.empty(0),
    ...
    ## Compute the terms
    n2_factor = (2*n_spheres)
    delta = center_2 - center_1 # ..., 3
    height = torch.linalg.vector_norm(delta, dim=-1, keepdim=True) # ..., 1
    m_idx = (2*torch.arange(n_spheres, dtype=center_1.dtype, device=center_1.device) + 1) # n_spheres

    radial_inner = radius_1.unsqueeze(-1) * n2_factor - m_idx * (radius_1-radius_2).unsqueeze(-1)
    radial_outer = torch.sqrt(height**2 + (radial_inner)**2)

    centers = center_1.unsqueeze(-2) + delta.unsqueeze(-2) * m_idx.unsqueeze(-1) / n2_factor # ..., n_spheres, 3
    radii = radial_outer / n2_factor # ..., n_spheres

    if bool_jacs:
        jacc = center_jac_1.unsqueeze(-3) + (m_idx / n2_factor)[..., None, None] * djac.unsqueeze(-3) # ..., n_spheres, 3, n_q
        jacr = torch.sum(delta.unsqueeze(-1) * djac, dim=-2, keepdim=True) / (n2_factor * radial_outer).unsqueeze(-1)
        return centers, radii, jacc, jacr
    return centers, radii, torch.empty(0), torch.empty(0)
