...
class KinematicUrdfWithObstacles(KinematicUrdfBase):
    ...
    def __init__(self,
        ...
        if len(self.obs_size_range.shape) != 3:
    ...
    def reset(self,
        ...
        '''
        # first reset the obstacle collision manager for free configuration collision checking
        self.obstacle_collision_manager = None

        # Setup the initial environment
        super().reset(**kwargs)

        # Generate the goal position, with a default retry count of 10
        self.reset_goal(qgoal=qgoal)
    ...
    def reset_goal(self, qgoal: np.ndarray = None, n_tries=10):
        '''Reset the goal position for the robot without resetting anything else in the environment
        ...
        Args:
            qgoal: np.ndarray, Optional
                ...
                If not provided, then a random goal position will be generated.
            n_tries: int
                Number of tries to generate a free configuration.
        ...
        if qgoal is not None:
            ...
            self.qgoal = self._generate_free_configuration(n_tries=n_tries)
        self.done = False
        self.terminal_observation = None
        self._info = None
        # reset the pyrender becuase I don't feel like tracking
        self.close()
        return self.get_observations()

    def _free_configuration_collision_check(self, fk_dict):
        '''Check for collisions with the obstacles and the robot
        ...
        '''
        collision = super()._free_configuration_collision_check(fk_dict)
        if not collision and self.obstacle_collision_manager is not None:
            temp_dist_flag = self.info_nearest_obstacle_dist
            self.info_nearest_obstacle_dist = True
            collision = self._collision_check_obstacle(fk_dict)[2] <= self.obs_gen_buffer
            self.info_nearest_obstacle_dist = temp_dist_flag
        return collision
    ...
    def add_obstacle(self, position: np.ndarray, size: np.ndarray):
        ...
        if len(self.obs_pos) > 0:
            self.obs_pos = np.concatenate([self.obs_pos, np.expand_dims(position,0)])
            self.obs_size = np.concatenate([self.obs_size, np.expand_dims(size,0)])
        else:
            self.obs_pos = np.expand_dims(position, 0)
            self.obs_size = np.expand_dims(size, 0)
        obs = trimesh.primitives.Box(extents=size)
        pose = np.eye(4)
        pose[0:3,3] = position
        obs_name = f'obs{len(self.obs_pos)-1}'
        self.obstacle_collision_manager.add_object(obs_name, obs, transform=pose)
        self.obs_meshes[obs_name] = (obs, pose)
    ...
    def collision_check(self, batch_fk_dict):
        ...
        for i in range(self.collision_discretization):
            time = float(i+1)/self.collision_discretization * self.t_step
