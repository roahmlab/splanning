...
from planning.sparrows.distance_net.compute_vertices_from_generators import compute_edges_from_generators
...
if __name__ == '__main__':
    ...
    parser.add_argument('scene_path', type=str, help="Path to experiment scene data")
    ...
    with open(f"{scene_path}/obstacles.pickle", 'rb') as f:
        obs_data = pickle.load(f)
        obs_pos = obs_data['centers']
        
        min_corner = (-1.389849, -1.939908, -0.5)
        max_corner = (1.904421, 1.621902, 1)
        
        mask = (obs_pos >= np.array(min_corner)) & (obs_pos <= np.array(max_corner))
        mask = mask.all(axis=1)
        obs_pos = obs_pos[mask]

        obs_size = obs_data['size']
        obs_size = np.tile(obs_size[None], [obs_pos.shape[0], 1])
        del obs_data
    ...
    for _ in tqdm.trange(n_steps):
        ...
        ka, flag, stats = planner.plan(qpos, qvel, wp, obstacles, ka_0=(ka if use_last_k else None), time_limit=10)
        print("Flag:", flag)
        if flag == 0:
            sphereviz.set_ka(ka)
        else:
