...
class KinematicUrdfBase:
    ...
    def __init__(self,
        ...
        for joint in self.robot.actuated_joints:
            if joint.joint_type == 'floating':
                ...
                eidx = sidx+1
            q_groups.append([sidx, eidx])
            sidx = eidx
        self.q_groups = q_groups if has_q_groups else None
        ...
        joint_idx = 0
        for joint in self.robot.actuated_joints:
            # no limits on floating or planar joints
            if joint.joint_type == 'floating':
                joint_idx += 6
                continue
            elif joint.joint_type == 'planar':
                joint_idx += 2
                continue
            # all other joint types take a single value
            elif joint.joint_type == 'continuous':
                self.continuous_joints.append(joint_idx)
            if joint.limit is not None:
                ...
                pos_lim[joint_idx] = [lower, upper]
                vel_lim[joint_idx] = joint.limit.velocity
                eff_lim[joint_idx] = joint.limit.effort
            joint_idx += 1
    ...
    def reset(self, qpos: np.ndarray = None, qvel: np.ndarray = 0, state: np.ndarray = None):
        '''Fully reset the environment and set the state of the robot to a given state or a random state.

        Clears the environment information and performs the reset.
        ...
        Returns:
            dict
                The observations of the environment after the reset.
        '''
        self.collision_info = None
        self.joint_limit_info = None

        return self.reset_start(qpos=qpos, qvel=qvel, state=state, n_tries=10)
    ...
    def reset_start(
        ...
        '''Reset the environment to a given state or a random state without clearing the environment information.

        Use this if you want to keep the environment configuration and just reset the state. Useful for downstream
        environments that need to reset the state but keep the environment configuration.
        ...
        if state is not None:
            ...
            if qpos is not None:
                ...
                self.qpos = self._generate_free_configuration(n_tries=n_tries)
                # TODO: Avoiding self collision with buffer or scale
                # self.qpos = self._generate_free_configuration(randomization_self_collision_scale=randomization_self_collision_scale, n_tries=n_tries)
    ...
    def to_q_list(self, q):
        '''From a given q-type, return a list of grouped q's for floating and planar joints.'''
        if self.q_groups is None:
            return q
        return [q[..., sidx:eidx].squeeze() for sidx, eidx in self.q_groups]

    def _free_configuration_collision_check(self, fk_dict):
        '''Check for collisions in a free configuration of the robot with some buffer (Not Implemented).
        
        Currently just returns the self collision check. Extend this in the subclass to add more checks,
        such as collision with the environment.
        ...
        Returns:
            bool
                Whether there is a collision in the given configuration.
        '''
        # def get_center(name):
        #     try:
        #         return self.robot_collision_manager_bb._objs[name]['geom'].aabb_center
        #     except:
        #         return np.zeros(3)
        self_collision_fk_dict = fk_dict
        # if randomization_self_collision_scale != 1.0:
        #     self_collision_fk_dict = {
        #         # doesn't work because FCL normalizes the rotations
        #             name: transform @ trimesh.transformations.scale_matrix(randomization_self_collision_scale, origin=get_center(name))
        #             for name, transform in fk_dict.items()
        #         }
        collision, _ = self._self_collision_check(self_collision_fk_dict, use_bb=True)
        return collision

    def _generate_free_configuration(self, n_tries=10, inf_bound=3*np.pi):
        '''Generate a random configuration for the robot that is not in self collision.
        
        Args:
            n_tries: int
                The number of times to try to generate a free configuration.
            inf_bound: float | np.array
                The bound to use for infinite joint limits in randomization
        ...
        pos_lim[np.isneginf(pos_lim)] = -inf_bound
        pos_lim[np.isposinf(pos_lim)] = inf_bound
        ...
        def pos_helper():
            ...
            collision = self._free_configuration_collision_check(fk_dict)
        ...
        for _ in range(n_tries):
            ...
            if not collision:
                ...
                return new_pos
        raise RuntimeError('Unable to find a free configuration in {} tries'.format(n_tries))
    ...
    def get_observations(self):
        ...
        if self._last_action is not None:
            observation['last_action'] = self._last_action
        return copy.deepcopy(observation)
    
    def step(self, action, save_full_trajectory = False):
        ...
        Args:
            ...
            save_full_trajectory: bool
                Whether to save the full trajectory of the action in the info dictionary. The full trajectory includes the position
                and velocity at each timestep_discretization. If using the integration step type, the full trajectory also includes
                the acceleration. Default is False.

        Returns:
            dict
                The observations of the environment after the step.
            float
                The reward for the step.
            bool
                Whether the episode is done.
            dict
                The info dictionary for the environment.
        '''
        ...
        # Return the observations, reward, done, and info
        if save_full_trajectory:
            full_trajectory = {"q": q, "qd": qd}
            if qdd is not None:
                full_trajectory["qdd"] = self._interpolate_q(qdd, self.timestep_discretization)
            self._info["full_trajectory"] = copy.deepcopy(full_trajectory)
        return observations, reward, self.done, self.info
    ...
    def _create_pyrender_scene(self, render_mesh, render_size, render_fps, renderer_kwargs):
        ...
        if self.renderer == RENDERER.PYRENDER:
            ...
            renderer_kwargs = copy.copy(renderer_kwargs)
            if renderer_kwargs.get('view_center') is not None:
                centroid = renderer_kwargs.pop('view_center')
            if renderer_kwargs.get('camera_pose') is not None:
                cp = renderer_kwargs.pop('camera_pose')
            elif renderer_kwargs.get('relative_camera_pose') is not None:
                cp = np.array(renderer_kwargs.pop('relative_camera_pose'))
                cp[:3,3] += centroid
            else:
    ...
    def last_action(self):
        '''Get a deepcopy of the last action taken in the environment.'''
